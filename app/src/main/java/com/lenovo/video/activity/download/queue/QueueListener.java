...
package com.lenovo.video.activity.download.queue;
...
import com.lenovo.video.activity.download.util.ProgressUtil;
...
import com.lenovo.video.activity.download.queue.QueueRecyclerAdapter.QueueViewHolder;
        ...
        if (status != null) {
            ...
            if (status.equals(EndCause.COMPLETED.toString())) {
                ...
                if (total == 0) {
                    ...
                    ProgressUtil.calcProgressToViewAndMark(holder.progressBar, TagUtil.getOffset(task), total, false);
                }
            }
        } else {
            // non-started
